...
class ListApplierNotificationAdapter(private val canConfirm:Boolean=false): ListAdapter<Freelancer, ListApplierNotificationAdapter.ListViewHolder>(DiffCallback()) {

    var onContactClickCallback:((String, String)->Unit)?=null

    var onConfirmApplyersClickCallback:((String)->Unit)?=null
        ...
        holder.binding.imgBtnOptionContact.setOnClickListener {v: View ->
            ...
            freelancer.contact?.let {
                showMenu(v, R.menu.popup_menu_contact, holder.itemView.context,
                    it, freelancer.idFreelancer.toString()
                )
            }
        }
    ...
    private fun showMenu(v: View, @MenuRes menuRes: Int, context: Context, contact: Contact, idFreelancer: String) {
        ...
        popup.setOnMenuItemClickListener { menuItem: MenuItem ->
            when(menuItem.itemId){
                ...
                R.id.menu_confirm_applyers ->{
                    onConfirmApplyersClickCallback?.invoke(idFreelancer)
                    true
                }
                else->{
                    false
